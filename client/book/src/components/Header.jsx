...
const Header = () => {
  ...
  const handleLogin = async () => {
    ...
    if(!name && !password) {
      ...
      try {
        const response = await axiosInstance.post(`https://neowriters.onrender.com/api/login`, {name, password});
  ...
  return (
    <div className='w-full flex flex-col md:flex-row items-center justify-between px-4 md:px-12 py-6 shadow relative' style={{color: isDarkMode && '#fff'}}>
      <div className="logo flex gap-4 md:gap-12">
       ...
       <div className='absolute -bottom-20 bg-gray-400 left-72 rounded-md shadow-md' style={{width: "500px", marginLeft: "40px"}}>
